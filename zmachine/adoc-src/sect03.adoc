...
****[1.0]* To define which characters are required, the Unicode (or ISO 10646-1) Basic Multilingual Plane character set is used: characters are specified by unsigned 16-bit codes.
...
****[1.0]* The story file chooses its stock of extra characters with a "`Unicode translation table`" as follows.
...
// @FIXME: "as shown in Table 1" -> refers to the first table in this Chapter,
//         which has no caption, but if we do add a caption its number would
//         be something other than 1 due to AsciiDoc auto-numbering.
//         Probably here it's best to add an X-Ref with the appropriate Table
//         number, even though it will differ from the original!
